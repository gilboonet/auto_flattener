/******************************************
 * DEPLIAGE v3                            *
 * - Modèle 3D :test simple (format .STL) *
 ******************************************/
include ("utils.jscad");
...
function main(params){
  ...
  a.V = a.lPts.map(function (el, iFace){return el.map(
            function (el2, iVoisin){
    let nV = chercheVoisin(a.lPts, iFace, iVoisin);
    return {T: nV, n1: null, n2: null,
            estCoP: estCoplanaire(a.fCsg[iFace], a.fCsg[nV]), 
            sens: classify(a.fCsg[iFace], a.fCsg[nV])
    }; }); });
    
  // Renseigne n1/n2
  for(let i =0; i< a.V.length; i++){
    for(let j =0; j< a.V[i].length; j++){
        let n = a.lPts[a.V[i][j].T].indexOf(a.lPts[i][j]);
        a.V[i][j].n2 = n;
...
/*function construitVolume(v){
  let r = v.lPts.map(m => CSG.Polygon.createFromPoints(m.map(n => v.lVtx[n])));
